...
```
#### Analysing Rapid facial mimicry
...
The `psycho` package also allows you to test the fit of the model and generates a description of the results using an `analyze` and then `summary` and `print` functions. 


```{r}
library(psycho)
results <- analyze(lmm_EMG)
```
...
Next step in the `psycho` process is to run contrasts. Here we are most interested in the interaction, so we can use `get_contrasts` for the interaction term, add them to the results and print them too.  
...
![](/post/2018-09-24-let-me-count-the-ways_files/lmerTest.jpg)
